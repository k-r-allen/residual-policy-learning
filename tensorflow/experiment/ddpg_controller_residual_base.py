...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
                 Q_lr, pi_lr_r, pi_lr_b, norm_eps, norm_clip, max_u, action_l2, clip_obs, scope, T,
                 rollout_batch_size, subtract_goals, relative_goals, clip_pos_returns, clip_return,
                 sample_transitions, gamma, base_only=True, residual_only=False,controller_type='hook', reuse=False, freeze=False, **kwargs):
    ...
    def _controller_action(self, o, g):
        push_controls = []
        obs = {'observation':None}
        for i in range(0, o.shape[0]):
            obs['observation'] = o[i,:]
            obs['desired_goal'] = g[i,:]
            if 'Hook' in self.info['env_name']:
                push_controls.append(get_hook_control(obs))
            elif 'Push' in self.info['env_name']:
                push_controls.append(get_push_control(obs))
            elif 'PickAndPlace' in self.info['env_name']:
                push_controls.append(get_pick_and_place_control(obs))
            else:
                print('ERROR: controller type not recognized')
        return np.stack(push_controls)
    ...
    def _grads(self):
        ...
        return critic_loss, actor_loss, Q_grad, pi_grad_r, pi_grad_b

    def _update(self, Q_grad, pi_grad_r, pi_grad_b, base, residual):
        self.Q_adam.update(Q_grad, self.Q_lr)
        if residual:
            self.pi_adam_r.update(pi_grad_r, self.pi_lr_r)
        if base:
            self.pi_adam_b.update(pi_grad_b, self.pi_lr_b)
    ...
    def update_target_net(self, base=True, residual=True):
        self.sess.run(self.update_target_net_op)
        if base:
            self.sess.run(self.update_target_net_op_b)
        if residual:
            self.sess.run(self.update_target_net_op_r)
    ...
    def _create_network(self, reuse=False):
        ...
        self.main_vars_r = self._vars('main/r_pi')
        self.main_vars_b = self._vars('main/b_pi')

        self.target_vars_r = self._vars('target/r_pi')
        self.target_vars_b = self._vars('target/b_pi')

        self.target_vars = self._vars('target/Q')# + self._vars('target/r_pi') + self._vars('target/b_pi')

        self.stats_vars = self._global_vars('o_stats') + self._global_vars('g_stats')

        self.init_target_net_op = list(
            map(lambda v: v[0].assign(v[1]), zip(self.target_vars+self.target_vars_r+self.target_vars_b, self.main_vars+self.main_vars_r+self.main_vars_b)))
        ...
        self.update_target_net_op_r = list(
            map(lambda v: v[0].assign(self.polyak * v[0] + (1. - self.polyak) * v[1]), zip(self.target_vars_r, self.main_vars_r)))

        self.update_target_net_op_b = list(
