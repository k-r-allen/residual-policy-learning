...
for j in ${!seeds[@]}; do
	...
	for i in ${!expertexplore[@]}; do
	  python train_staged.py --env ${expertexplore[$i]} --n_epochs $nepochs --num_cpu ${cpus[$i]} --config_path=configs/${configs[$i]} --logdir ./logs/seed${seeds[$j]}/${expertexplore[$i]}_expertexplore --seed ${seeds[$j]} --random_eps=${eps[$i]} --controller_prop=${alpha[$i]}
	done

	#Train from scratch
	for i in ${!envs[@]}; do
	  python train_staged.py --env ${envs[$i]} --n_epochs $nepochs --num_cpu ${cpus[$i]} --config_path=configs/${configs[$i]} --logdir ./logs/seed${seeds[$j]}/${envs[$i]} --seed ${seeds[$j]}
	done

	#Train residuals
	for i in ${!residuals[@]}; do
	  python train_staged.py --env ${residuals[$i]} --n_epochs $nepochs --num_cpu ${cpus[$i]} --config_path=configs/${configs[$i]} --logdir ./logs/seed${seeds[$j]}/${residuals[$i]} --seed ${seeds[$j]}
	done
done